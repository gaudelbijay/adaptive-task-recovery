...
| [`envs/tidy_up_replicacad_policies.py`](envs/tidy_up_replicacad_policies.py) | Same policy API, navigating (not just reaching) to each goal. `_TRAY_POSITION`/`_TRAY_HALF_SIZES` come from the lightweight shared layout module rather than being duplicated. | `spikes/task_schema_draft/policy_baselines_replicacad.py` | D-048 |
