...
| Person B | Implement static, oracle-feasibility, and adaptive policy baselines plus the intent guard — 2026-07-29/30: static vs oracle-feasibility policies done (D-014, H2 toy test); intent guard done (D-015, H3 toy test — blocks a constraint violation at zero recall cost, easy case only, see R-010 caveat); confirmed embodiment-agnostic on both a Unitree G1 humanoid (D-016) and a real ReplicaCAD apartment + Fetch mobile robot with real path planning (D-017) — same results all three times. Adaptive (learned) policy not started. |
