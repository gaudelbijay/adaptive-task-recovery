...
| Person B | Implement static, oracle-feasibility, and adaptive policy baselines plus the intent guard — 2026-07-29: static vs oracle-feasibility policies done (D-014, H2 toy test); intent guard done (D-015, H3 toy test — blocks a constraint violation at zero recall cost, easy case only, see R-010 caveat); confirmed embodiment-agnostic with a Unitree G1 humanoid variant (D-016, same results as panda). Adaptive (learned) policy not started. |
