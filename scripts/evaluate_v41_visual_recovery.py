#!/usr/bin/env python3
"""Evaluate frozen-threshold V41 with V40 interaction accounting."""
...
def install_protocol_adapter():
    ...
    def annotated(payload, path: Path):
        ...
        if completion.get("training_protocol") != "backkey_targeted_dense_repair_v19":
            raise ValueError("V41 requires the audited targeted-dense checkpoint")
        local = int(completion["dense_finetune_transitions"])
        if int(payload["checkpoint_global_step"]) != local: raise ValueError("V41 accounting mismatch")
        ...
        payload["evaluation_source_sha256"]["v41_protocol_adapter"] = hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
