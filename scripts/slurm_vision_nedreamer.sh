...
if [[ "${ATR_COMPLETE}" != "1" ]]; then
  sbatch --array="${ATR_TASK_INDEX}" \
    --export="ALL,ATR_NE_CONFIG=${ATR_NE_CONFIG},ATR_NE_OUTPUT=${ATR_NE_OUTPUT},ATR_NE_UPSTREAM=${ATR_NE_UPSTREAM},ATR_PYTHON=${ATR_PYTHON}" \
    scripts/slurm_vision_nedreamer.sh
fi
