#!/usr/bin/env python3
"""Annotate V28 evaluation without changing the frozen shared evaluator."""
...
def install_protocol_adapter() -> None:
    ...
    def annotated_atomic_json(payload: dict, path: Path) -> None:
        ...
        if completion.get("training_protocol") != "paired_rendered_domain_distillation":
            raise ValueError("V28 evaluation requires rendered-domain distillation provenance")
        ...
        if int(payload["checkpoint_global_step"]) != student:
            raise ValueError("V28 checkpoint/student-transition accounting mismatch")
        for key in (
