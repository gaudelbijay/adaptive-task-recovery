#!/usr/bin/env bash
#SBATCH --job-name=atr-state-continuation
...
#SBATCH --requeue
#SBATCH --output=results/slurm/state_continuation_%A_%a.out
#SBATCH --error=results/slurm/state_continuation_%A_%a.err
...
"${ATR_PYTHON}" scripts/train_manipulation_continuation_ppo.py \
  --config "${ATR_RECOVERY_CONFIG}" \
  --output "${ATR_RECOVERY_OUTPUT}" \
  --task-index "${SLURM_ARRAY_TASK_ID}"

ATR_TASK_STDERR="results/slurm/state_continuation_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err"
