#!/usr/bin/env bash
#SBATCH --job-name=atr-v41-strict-removal
#SBATCH --partition=gpu-l40s
#SBATCH --gres=gpu:l40s:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=02:00:00
#SBATCH --output=results/slurm/v41_strict_eval_%A_%a.out
#SBATCH --error=results/slurm/v41_strict_eval_%A_%a.err

set -euo pipefail
: "${ATR_VISUAL_CONFIG:?set ATR_VISUAL_CONFIG}"
mkdir -p results/slurm
export PYTHONUNBUFFERED=1
.venv/bin/python scripts/evaluate_v41_visual_recovery_strict_removal.py \
