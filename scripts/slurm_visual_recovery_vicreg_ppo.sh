#!/usr/bin/env bash
#SBATCH --job-name=atr-visual-vicreg
...
#SBATCH --requeue
#SBATCH --output=results/slurm/visual_vicreg_%A_%a.out
#SBATCH --error=results/slurm/visual_vicreg_%A_%a.err
...
"${ATR_PYTHON}" scripts/train_visual_recovery_vicreg_ppo.py \
