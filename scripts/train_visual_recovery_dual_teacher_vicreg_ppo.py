#!/usr/bin/env python3
"""Checkpointed RGB PPO with routed teachers and VICReg regularization.
...
temporal latent prediction plus variance/covariance regularization supplies an
anti-collapse self-supervised representation loss.  Actions use a
...
def vicreg_regularization(latent: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """VICReg variance/covariance penalties over independent rollout samples."""
    if latent.ndim != 2 or latent.shape[0] < 2:
        raise ValueError("VICReg requires a 2-D minibatch with at least two samples")
    centered = latent - latent.mean(dim=0, keepdim=True)
    standard_deviation = torch.sqrt(centered.var(dim=0, unbiased=True) + 1e-4)
    variance_loss = F.relu(1.0 - standard_deviation).mean()
        ...
        for _ in range(int(config["update_epochs"])):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                variance_coefficient = float(
                    task.get("temporal_variance_coefficient", 0.0)
                )
                covariance_coefficient = float(
                    task.get("temporal_covariance_coefficient", 0.0)
                )
                if variance_coefficient or covariance_coefficient:
                    variance_loss, covariance_loss = vicreg_regularization(latent)
                ...
                loss = (
                    ...
                    + variance_coefficient * variance_loss
                ...
                for key, value in (
                    ...
                    ("variance", variance_loss), ("covariance", covariance_loss),
                ):
