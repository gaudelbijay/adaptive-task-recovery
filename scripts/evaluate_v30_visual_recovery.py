#!/usr/bin/env python3
"""Annotate V30 evaluation without changing frozen shared evaluators."""
...
def install_protocol_adapter() -> None:
    """Replace legacy PPO accounting with V30's actual DAgger accounting."""
    ...
    def annotated_atomic_json(payload: dict, path: Path) -> None:
        ...
        if completion.get("training_protocol") != "full_episode_multidomain_dagger":
            raise ValueError("V30 evaluation requires multidomain DAgger provenance")
        ...
        if int(payload["checkpoint_global_step"]) != student:
            raise ValueError("V30 checkpoint/DAgger-transition accounting mismatch")
        for key in (
        ...
        payload["evaluation_source_sha256"]["v30_protocol_adapter"] = hashlib.sha256(
