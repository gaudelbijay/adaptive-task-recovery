#!/usr/bin/env python3
"""Fresh-seed safe-recovery audit for a frozen external Peg specialist."""

from __future__ import annotations

import argparse
import hashlib
import importlib
import json
from pathlib import Path

import gymnasium as gym
import numpy as np
import torch

import mani_skill.envs  # noqa: F401
from mani_skill.utils.wrappers.flatten import FlattenActionSpaceWrapper
from mani_skill.vector.wrappers.gymnasium import ManiSkillVectorEnv

from train_manipulation_ppo import Agent


@torch.inference_mode()
def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--checkpoint", type=Path, required=True)
    parser.add_argument("--episodes", type=int, default=192)
    parser.add_argument("--num-envs", type=int, default=64)
    parser.add_argument("--steps", type=int, default=160)
    parser.add_argument("--seed-base", type=int, default=421_600_000)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    if args.episodes % args.num_envs:
        raise ValueError("episodes must be divisible by num-envs")
    device = torch.device("cuda")
    checkpoint = torch.load(args.checkpoint, map_location=device, weights_only=False)
    task = checkpoint["task"]
    if task.get("registration_module"):
        importlib.import_module(task["registration_module"])
    kinds = tuple(task["env_kwargs"]["intervention_types"])
    if len(kinds) != 1 or kinds[0] not in {
        "positive_lateral_peg_ejection", "negative_lateral_peg_ejection",
    }:
        raise ValueError("checkpoint is not a single-direction ejection specialist")
    condition = kinds[0]
    safe_successes = violations = finished = 0
    action_dim = observation_dim = None
    agent = None
    for batch in range(args.episodes // args.num_envs):
        env = gym.make(
            "PegInsertionRecovery-v1", num_envs=args.num_envs,
            reconfiguration_freq=1, obs_mode="state", render_mode=None,
            sim_backend="physx_cuda", control_mode=task["control_mode"],
            reward_mode="normalized_dense", max_episode_steps=args.steps,
            intervention_probability=1.0, intervention_types=(condition,),
            onset_step_range=(18, 42), include_blocker_state_observation=False,
            ejection_force=12.0, ejection_steps=30,
            ejection_target_displacement=0.06,
            ejection_position_gain=160.0, ejection_velocity_gain=10.0,
        )
        if isinstance(env.action_space, gym.spaces.Dict):
            env = FlattenActionSpaceWrapper(env)
        env = ManiSkillVectorEnv(
            env, args.num_envs, ignore_terminations=True, record_metrics=False,
        )
        try:
            observation, _ = env.reset(seed=args.seed_base + batch)
            if agent is None:
                observation_dim = int(np.prod(env.single_observation_space.shape))
                action_dim = int(np.prod(env.single_action_space.shape))
                agent = Agent(observation_dim, action_dim).to(device)
                agent.load_state_dict(checkpoint["agent"], strict=True)
                agent.eval()
            action_low = torch.as_tensor(env.single_action_space.low, device=device)
            action_high = torch.as_tensor(env.single_action_space.high, device=device)
            success_once = torch.zeros(args.num_envs, dtype=torch.bool, device=device)
            violation_once = torch.zeros_like(success_once)
            finished_once = torch.zeros_like(success_once)
            for _ in range(args.steps):
                action = torch.clamp(
                    agent.get_action(observation, deterministic=True),
                    action_low, action_high,
                )
                observation, _, _, _, info = env.step(action)
                violation_once |= info["constraint_violated"].bool()
                finished_once |= info["intervention_finished"].bool()
                success_once |= info["success"].bool() & finished_once
            safe_successes += int((success_once & ~violation_once).sum())
            violations += int(violation_once.sum())
            finished += int(finished_once.sum())
        finally:
            env.close()
    result = {
        "schema_version": 1,
        "audit": "external_peg_direction_specialist",
        "condition": condition,
        "training_seed": int(task["seed"]),
        "evaluation_seed_base": args.seed_base,
        "episodes": args.episodes,
        "safe_successes": safe_successes,
        "safe_success_rate": safe_successes / args.episodes,
        "violations": violations,
        "violation_rate": violations / args.episodes,
        "intervention_finished": finished,
        "checkpoint": str(args.checkpoint),
        "checkpoint_global_step": int(checkpoint["global_step"]),
        "checkpoint_sha256": hashlib.sha256(args.checkpoint.read_bytes()).hexdigest(),
        "runtime_pose_assignment": False,
    }
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    print(json.dumps(result, indent=2, sort_keys=True))


if __name__ == "__main__":
    main()
