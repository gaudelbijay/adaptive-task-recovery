#!/usr/bin/env python3
"""Evaluate V42 broad-render dense repair with explicit accounting."""
...
import v41_magnitude_gated_agent as v41


def install_protocol_adapter():
    original_atomic_json = base.atomic_json
    base.VisualAgent = v41.V41MagnitudeGatedDenseV19Agent

    def annotated(payload, path: Path):
        completion = json.loads((path.parent / "TRAINING_COMPLETE.json").read_text())
        if completion.get("training_protocol") != "broad_render_dense_repair_v19":
            raise ValueError("V42 requires the audited broad-render checkpoint")
        local = int(completion["dense_finetune_transitions"])
        if int(payload["checkpoint_global_step"]) != local:
            raise ValueError("V42 accounting mismatch")
        for key in (
        ...
        payload["magnitude_threshold"] = 0.015
        payload["evaluation_source_sha256"]["v42_protocol_adapter"] = hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
