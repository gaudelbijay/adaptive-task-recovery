#!/usr/bin/env bash
#SBATCH --job-name=atr-visual-shift-action
...
#SBATCH --requeue
#SBATCH --output=results/slurm/visual_shift_action_%A_%a.out
#SBATCH --error=results/slurm/visual_shift_action_%A_%a.err

set -euo pipefail

ATR_VISUAL_CONFIG="${ATR_VISUAL_CONFIG:-configs/visual_recovery_dual_specialist_shift_action_v24_runtime_smoke.json}"
...
ATR_NODE_CACHE="${SLURM_TMPDIR:-/tmp}/atr-visual-shift-action-${SLURM_JOB_ID}-${ATR_TASK_INDEX}"
...
"${ATR_PYTHON}" scripts/train_visual_recovery_dual_teacher_shift_action_ppo.py \
