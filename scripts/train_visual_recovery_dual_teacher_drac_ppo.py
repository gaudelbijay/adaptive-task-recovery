#!/usr/bin/env python3
"""Checkpointed dual-teacher RGB PPO with DrAC policy consistency.
...
clipped sample. PPO ratios are always computed from original observations;
random shifts appear only in a stopped-target policy KL and optional temporal
representation target, avoiding augmented-observation importance ratios.
"""
...
from drac_policy_consistency import drac_policy_consistency
...
_SOURCE_PATHS = {
    ...
    "drac_policy_consistency": Path(__file__).with_name("drac_policy_consistency.py"),
}
SOURCE_SHA256 = {
...
def main():
    ...
    augmentation_pad = int(task.get("augmentation_pad", 0))
    drac_coefficient = float(task.get("drac_policy_coefficient", 0.0))
    if drac_coefficient <= 0:
        raise ValueError("DrAC trainer requires a positive policy coefficient")
    if augmentation_pad <= 0:
        raise ValueError("DrAC trainer requires positive random-shift padding")
    if not task.get("asymmetric_critic", False):
        raise ValueError("this DrAC protocol requires the frozen asymmetric state critic")
        ...
        for _ in range(int(config["update_epochs"])):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                _, action, new_logprob, entropy, new_value, latent = agent.action_and_value(
                    b_rgb[mb], b_prop[mb], b_critic[mb], b_pre[mb], augment=False,
                )
                ...
                drac_loss = drac_policy_consistency(
                    agent, b_rgb[mb], b_prop[mb],
                )
