#!/usr/bin/env python3
"""Checkpointed state PPO with immutable specialist initialization support.
    ...
    if latest_path.exists():
        ...
        if not initialization_path.exists():
            raise FileNotFoundError(
                f"state initializer unavailable: {initialization_path}"
            )
        ...
        initialization_record = {
            ...
            "checkpoint_sha256": _file_sha256(initialization_path),
            "source_task": source_task,
            "source_global_step": int(initialization["global_step"]),
            "optimizer_reinitialized": True,
        }
        temporary = run_dir / f".initialization.json.tmp.{os.getpid()}"
        temporary.write_text(
            json.dumps(initialization_record, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
