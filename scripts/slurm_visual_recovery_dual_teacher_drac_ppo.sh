#!/usr/bin/env bash
#SBATCH --job-name=atr-visual-dual-drac
...
#SBATCH --requeue
#SBATCH --output=results/slurm/visual_dual_drac_%A_%a.out
#SBATCH --error=results/slurm/visual_dual_drac_%A_%a.err

set -euo pipefail

ATR_VISUAL_CONFIG="${ATR_VISUAL_CONFIG:-configs/visual_recovery_dual_specialist_drac_v22_smoke.json}"
...
ATR_NODE_CACHE="${SLURM_TMPDIR:-/tmp}/atr-visual-drac-${SLURM_JOB_ID}-${ATR_TASK_INDEX}"
...
"${ATR_PYTHON}" scripts/train_visual_recovery_dual_teacher_drac_ppo.py \
