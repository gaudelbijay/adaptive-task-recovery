...
class Agent(nn.Module):
    def __init__(
        ...
        actor_logstd_initial: float = -0.5,
        ...
        self.actor_logstd = nn.Parameter(torch.full(
            (1, action_dim), float(actor_logstd_initial),
        ))
...
def main():
    ...
    agent = Agent(
        ...
        actor_logstd_initial=float(config.get("actor_logstd_initial", -0.5)),
