...
Last updated: 2026-08-22 (D-121)

## Active

| ID | Type | Severity | Description | Mitigation / next check |
|---|---|---|---|---|
| R-015 | Risk | High | Benchmark shortcuts are pervasive and each fix can create another. Four independent instances are now recorded (D-232--D-237): `LearnedRecovery-v4` identifies its held-out mechanism from separate ejection actors; current-centering removed an instantaneous shortcut and created a one-frame one; REBOOT's verdict depends on which non-recurrent control is nominated; PegInsertion leaks blockage identity through `normalized_time`. | Run `scripts/audit_shortcut_ladder.py` with a matched rung set before any composition claim, and ablate any time or duration feature. Treat a verdict computed against a single weak control as unsound. |
| R-016 | Risk | High | Results on `LearnedRecovery-v4` do not transfer. The permanent/temporary finding reverses on PegInsertion, where the memoryless arm is best on permanent blockage, and the factorized architecture has no external support anywhere. | State every v4 finding as scoped to v4 unless replicated. Do not present the architecture as a contribution; it is a disclosed negative. |
| R-017 | Risk | Medium | The benchmark that the audit flags is an easy task with scripted interventions: 5 cm cubes, 4 cm tolerance, exogenous events at step 0. Recovery here is goal filtering rather than recovery from execution failure. | Stated in `../README.md` scope and `../docs/19-iros-publishability-gate.md`. Emergent failures were attempted and found 98.8% single-mode (D-234), so the route needs task geometry changes rather than more training. |
| R-005 | Risk | High | “Feasibility” may collapse into detecting intervention labels rather than estimating reachability. Concretely present in the current draft: `goal_feasible()` in `src/atr/feasibility/oracle.py` (promoted from `spikes/task_schema_draft/oracle_feasibility.py`, D-037) is a direct object-existence query — functionally almost identical to detecting the intervention label itself, not an estimate of reachability. Acceptable for now only because it's a privileged-state oracle used to validate plumbing (D-014), not a learned feasibility model. Still true after D-020/D-023: those give a real (imperfect) *perceptual* feasibility signal instead of privileged state, which is the actual mitigation this risk calls for, but only for the two objects/scenes calibrated so far. **Confirmed still scene-specific 2026-08-23 (D-123):** attempted CLIP calibration on D-121's third layout rather than assuming it would transfer, and found it genuinely fails there (real robot occlusion of the target region, candidate margins staying positive whether the object is present or absent) -- privileged-state generalization (D-122) does not imply perceptual generalization, confirmed rather than assumed. `visual_object_exists()` raises explicitly for this scene_variant instead of silently guessing. | Use reversible/neutral controls, counterfactual pairs, and oracle-regret evaluation. When a learned feasibility estimator is built, it must not have direct access to the "did an intervention fire" signal, or this risk reproduces exactly. |
...
**R-014 mechanism found (D-116, 2026-08-14):** per the user's explicit choice
to investigate this gap next, read `ReplicaCADRearrangeSceneBuilder`'s real
source (`build()`/`initialize()`) instead of another round of standalone-vs-
production comparison, per D-061's own recommendation. Found that
`_OBJECT_ALIASES`' hardcoded YCB actor-instance suffixes (tuned for
`build_config_idx=59`/`55`) are not portable: for D-061's candidate `13`,
the hardcoded suffix resolves to a hidden instance, and the real placed
instance is a different number entirely, at a position not near the scene's
other real objects. Verified directly with an instrumented script, not
inferred. This is a data/content-level bug (per-scene instance numbering),
explaining why D-061's exhaustive runtime/process-level checks (seed,
`PYTHONHASHSEED`, import order, subprocess isolation) never found it — none
of them was ever the actual cause. Fix identified (resolve aliases
dynamically per scene) but not implemented; `build_config_idx=13` itself
remains unusable regardless, since its real object positions are too spread
...
**A real third layout found and verified (D-121, 2026-08-22):** per the
user's explicit choice to search for one next, checked real placed
positions directly across all 61 other valid `build_config_idx` values at
`torch_seed=0` -- D-116's own recommendation, not another raycast/visual
proxy (D-061's actual mistake). Two independent filters: object-pair
proximity, then real floor-clearance raycasting at the resulting midpoint.
This caught a real trap a single-signal search would have missed:
`build_config_idx=31` had the closest `master_chef_can`/`potted_meat_can`
pair (0.14m) of any candidate, but the standing spot there is fully
enclosed (12/12 raycast hits). `build_config_idx=17` clears both filters
and, checked separately again, keeps `bowl`/`cracker_box` within ~1.1m of
the same point too -- most other candidates left one of those two several
meters away. `cracker_box` needed a second fix beyond D-119: no hardcoded
value exists to preserve for a brand-new layout, so it resolves
dynamically for any `scene_variant` outside the two legacy ones. Confirmed
non-degenerate with a real subprocess-isolated render capture before
finalizing the camera config. New `scene_variant="third_layout"`; 5 new
tests (`TestThirdSceneLayout`) including a real end-to-end
`static`-vs-`feasibility_aware` policy comparison, not just placement
checks; all 15 tests in the file pass. Explicitly privileged-state only --
`clip_feasibility.py` crops not calibrated. Held-out-scene-layout is built
but not yet *exercised* (no split registry entry, no train-on-two-
held-out-one comparison) -- deliberately left open, same as D-059/D-069's
split.
